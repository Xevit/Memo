...
\newglossaryentry{rrss}{name=RRSS, description={Redes Sociales}}
...
\newglossaryentry{ti}{name=TI, description={Tecnologías de la Información}}
\newglossaryentry{pki}{name=PKI, description={Public Key Infrastructure}}
\newglossaryentry{oasis}{name=OASIS, description={Open Architecture Scientific Information System}}
...
\newglossaryentry{msnbc}{name=MSNBC, description={Canal de noticias estadounidense}}
\newglossaryentry{bbdd}{name=BBDD, description={Base de Datos}}
\newglossaryentry{json}{name=JSON, description={JavaScript Object Notation}}
\newglossaryentry{otp}{name=OTP, description={One Time Password}}
...
A partir de los avances tecnológicos actuales podemos predecir que las contraseñas ya no son un método confiable de autenticación de usuario. Este problema, combinado con la creciente amenaza de máquinas malintencionadas, hace que muchos expertos en \gls{ti} se pregunten como pueden asegurarse de que solo los usuarios y dispositivos aprobados tengan acceso. La solución a este problema lo podemos encontrar en los certificados electrónicos.
...
\begin{enumerate}
	...
	\item El proveedor de identidad construye la respuesta de autenticación en forma de un documento \gls{xml} que contiene el nombre de usuario o la dirección de correo electrónico del usuario, lo firma con un certificado X.509 y publica esta información al proveedor de servicios.
...
El estándar OAuth 2.0~\cite{hardt2012oauth} (\gls{rfc} 6749)~\cite{hardt2012rfc} es un \emph{framework} de autorización que permite controlar el acceso por parte de las aplicaciones a los datos de los usuarios sin tener que proporcionar las credenciales. Según el estándar, el marco de autorización de OAuth 2.0 permite que una aplicación de terceros obtenga acceso limitado a un servicio \gls{http}, ya sea en nombre de un propietario de recursos mediante la organización de una interacción de aprobación entre el propietario del recurso y el servicio \gls{http}, o permitiendo a la aplicación de terceros obtener acceso en su propio nombre. 
...
Este trabajo fin de grado ha consistido en crear los mecanismos necesarios para la concesión de autorización entre una aplicación y una \gls{api} a través de una implementación del estándar OAuth 2.0.
...
\begin{enumerate}
	...
	\item Creación del proceso de registro de aplicaciones en la aplicación: Creación de la funcionalidad específica de alta y baja de aplicaciones en la \gls{api}. De esta forma la aplicación podrá solicitar acceso a los recursos de los usuarios a la \gls{api}.
	\item Creación del proceso de autorización utilizando el estándar OAuth 2.0: Desarrollo de la funcionalidad específica del estándar OAuth 2.0 basándonos en la documentación establecida en la \gls{rfc} 6749~\cite{hardt2012rfc}.
	\item Creación del \emph{front} básico de gestión de usuarios de la \gls{api}: Dicha web deberá disponer de una interfaz simple que permita realizar la autenticación de un usuario en la \gls{api}.
\end{enumerate}
...
\subsection{Gradle}
\label{subsec:gradle}
Gradle\footnote{\url{https://gradle.org/}} es una herramienta de automatización de compilación de código abierto centrada en la flexibilidad y el rendimiento. Los scripts de compilación de Gradle se escriben utilizando un \gls{dsl} Groovy o Kotlin.
\begin{enumerate}
...
\subsection{JQuery}
\label{subsec:jquery}
jQuery\footnote{\url{https://jquery.com/}} es una biblioteca de JavaScript rápida, pequeña y con muchas funciones. Hace que cosas como la manipulación de documentos HTML, el manejo de eventos, la animación y Ajax sean mucho más simples con una \gls{api} fácil de usar que funciona en una gran cantidad de navegadores. Con una combinación de versatilidad y extensibilidad, jQuery ha cambiado la forma en que millones de personas escriben JavaScript.

\subsection{Ajax}
\label{subsec:ajax}
Ajax\footnote{\url{https://web.archive.org/web/20080413132458/http://www.librosweb.es/ajax/index.html}} acrónimo de Asynchronous JavaScript And \gls{xml}~\cite{eernisse2006build} (JavaScript asíncrono y \gls{xml}), es una técnica de desarrollo web para crear aplicaciones interactivas o RIA (Rich Internet Applications). Estas aplicaciones se ejecutan en el cliente, es decir, en el navegador de los usuarios mientras se mantiene la comunicación asíncrona con el servidor en segundo plano. De esta forma es posible realizar cambios sobre las páginas sin necesidad de recargarlas, mejorando la interactividad, velocidad y usabilidad en las aplicaciones.
...
Bootstrap es el segundo proyecto más destacado en GitHub y es usado por la \gls{nasa} y la \gls{msnbc} entre otras organizaciones.
...
\subsection{REST}
\label{subsec:rest}
La transferencia de estado representacional (en inglés representational state transfer) o \gls{rest}, es un estilo de arquitectura software para sistemas hipermedia distribuidos como la World Wide Web.
...
\begin{itemize}
	...
	\item Cliente: Aplicación que quiere acceder a la cuenta del usuario. Antes de que pueda hacerlo deber ser autorizado por el usuario, y la autorización debe ser validada por la \gls{api}.
...
\begin{enumerate}
	...
	\end{itemize}
	\item Creación del proceso de registro de aplicaciones en la Aplicación: Creación de la funcionalidad específica de alta y baja de aplicaciones en la \gls{api}:
	\begin{itemize}
		...
		\item Consistirá en el desarrollo de dos botones. Uno de ellos tendrá la capacidad de realizar el proceso de comunicación entre la aplicación y la \gls{api} para registrar dicha aplicación en la lista de confianza de la \gls{api}.
		\item El segundo botón tendrá la capacidad de realizar el proceso de comunicación entre la aplicación y la \gls{api} para eliminar nuestra aplicación de la lista de confianza de la \gls{api}. Este botón se usará en el caso en el que no queramos que nuestra aplicación figure en la lista de confianza de la \gls{api}.
	\end{itemize}
	\item Creación del proceso de autorización utilizando el estándar OAuth 2.0: Desarrollo de la funcionalidad específica del estándar OAuth 2.0 basándonos en la documentación establecida en la \gls{rfc} 6749~\cite{hardt2012rfc}: 
	\begin{itemize}
		\item Partes implicadas $\rightarrow$ \textbf{Aplicación - Usuario - API}.
		\item Esta funcionalidad funcionará correctamente solamente en caso de que previamente hayamos dado de alta nuestra aplicación en la lista de confianza de la \gls{api} (ver punto 2).
		\item Consistirá en la creación de un botón que desarrolle la funcionalidad completa del protocolo OAuth 2.0.
		\item Dentro de los pasos necesarios para la correcta ejecución del protocolo se encuentra la de mostrar una ventana para que el usuario se autentique contra la plataforma de la \gls{api}. En caso de no estar dado de alta previamente en dicha \gls{api} u obtener una autenticación fallida el proceso completo dará error y no se obtendrá token de acceso.
	\end{itemize}
\end{enumerate}
...
La arquitectura de base de datos(\gls{bbdd}) consta de dos partes diferenciadas. Por un lado tenemos las tablas de la \gls{bbdd} de la aplicación (REDOA2) y por otro lado las tablas de la \gls{api} (APIOA2). 

\subsection{Base de datos de la aplicación}
\label{subsec:bbdd_aplicacion}
Las tablas correspondientes a la aplicación son las siguientes:
\subsubsection{REDOA2\_USERS}
\label{subsubsec:redoa2_users}
La figura \ref{figura:redoa2_users} es un ejemplo de la forma que va a tener la \gls{bbdd}.

\begin{figure}[H]
	\includegraphics[scale=1, center]{image/4_4_1_Red_1.PNG}
	\caption{RedOA2\_Users}
	\label{figura:redoa2_users}
\end{figure}

En esta \gls{bbdd} se van a almacenar los datos de los usuarios que se registren en la aplicación.
\begin{itemize}
	\item \textbf{id}: Identificador del número de fila en el que se encuentra este registro.
	\item \textbf{nombre}: Nombre del usuario que se ha registrado en la plataforma.
	\item \textbf{usuario}: Usuario de acceso a la plataforma. En caso de realizar el loguin se deberá indicar este usuario.
	\item \textbf{password}: Contraseña de acceso a la plataforma. En caso de realizar el loguin se deberá indicar este password.
	\item \textbf{email}: Email de contacto del usuario registrado.
\end{itemize}

\subsubsection{REDOA2\_APPLICATION\_REQUEST}
\label{subsubsec:redoa2_application_request}

La figura \ref{figura:redoa2_application_request} es un ejemplo de la forma que va a tener la \gls{bbdd}.

\begin{figure}[H]
	\includegraphics[scale=0.9, center]{image/4_4_1_Red_2.PNG}
	\caption{RedOA2\_Application\_Request}
	\label{figura:redoa2_application_request}
\end{figure}

Una vez que se ha realizado el alta de una nueva aplicación en la API se nos facilitan los datos de la siguiente tabla.
\begin{itemize}
	\item \textbf{url\_redirect}: URL a la que se tiene que enviar la respuesta en la comunicación.
	\item \textbf{code\_secret}: Código enviado al correo de la aplicación que quiere darse de alta en la API como factor primario de autenticación. Se almacena en Base 64.
	\item \textbf{client\_id}: El CLIENT\_ID lo emite la API y actúa como usuario a la hora de solicitar recursos.
	\item \textbf{client\_secret}: El CODE\_SECRET lo emite la API y actúa como una contraseña a la hora de solicitar recursos.
\end{itemize}

\subsubsection{REDOA2\_APPLICATION\_RECORD}
\label{subsubsec:redoa2_application_record}

La figura \ref{figura:redoa2_application_record} es un ejemplo de la forma que va a tener la \gls{bbdd}.

\begin{figure}[H]
	\includegraphics[scale=0.8, center]{image/4_4_1_Red_3.PNG}
	\caption{RedOA2\_Application\_Record}
	\label{figura:redoa2_application_record}
\end{figure}
...
\begin{itemize}
	...
	\item \textbf{token\_type}: Token que indica que dicho token es de actualización y que se podrá utilizar para solicitar un nuevo token en el momento en el que se caduque.
	\item \textbf{expires\_in}: Tiempo de expiración del Token.
	\item \textbf{refresh\_token}: Token de actualización que servirá para solicitar un nuevo 'ACCESS\_TOKEN' en el momento en que se supere el 'EXPIRES\_IN'
\end{itemize}

\subsection{Base de datos de la API}
\label{subsec:bbdd_api}
Las tablas correspondientes a la API son las siguientes:

\subsubsection{APIOA2\_USERS}
\label{subsubsec:apioa2_users}

La figura \ref{figura:apioa2_users} es un ejemplo de la forma que va a tener la \gls{bbdd}.

\begin{figure}[H]
	\includegraphics[scale=0.8, center]{image/4_4_2_Red_1.PNG}
	\caption{APIOA2\_Users}
	\label{figura:apioa2_users}
\end{figure}

En esta \gls{bbdd} se van a almacenar los datos de los usuarios que se registren en la API.
\begin{itemize}
	\item \textbf{id}: Identificador del número de fila en el que se encuentra este registro.
	\item \textbf{nombre}: Nombre del usuario que se ha registrado en la plataforma.
	\item \textbf{usuario}: Usuario de acceso a la plataforma. En caso de realizar el loguin se deberá indicar este usuario.
...
\subsubsection{APIOA2\_APPLICATION\_REQUEST}
\label{subsubsec:apioa2_application_request}

La figura \ref{figura:apioa2_application_request} es un ejemplo de la forma que va a tener la \gls{bbdd}.

\begin{figure}[H]
	\includegraphics[scale=1, center]{image/4_4_2_Red_2.PNG}
	\caption{APIOA2\_Application\_Request}
	\label{figura:apioa2_application_request}
\end{figure}
...
\begin{itemize}
	...
	\item \textbf{code\_secret}: Código enviado al correo de la aplicación que quiere darse de alta en la API como factor primario de autenticación. Se almacena en Base 64.
	\item \textbf{client\_id}: El CLIENT\_ID lo emite la API y actúa como usuario a la hora de solicitar recursos.
	\item \textbf{client\_secret}: El CLIENT\_SECRET lo emite la API y actúa como una contraseña a la hora de solicitar recursos.
\end{itemize}

\subsubsection{APIOA2\_APPLICATION\_RECORD}
\label{subsubsec:apioa2_application_record}

La figura \ref{figura:apioa2_application_record} es un ejemplo de la forma que va a tener la \gls{bbdd}.

\begin{figure}[H]
	\includegraphics[scale=0.8, center]{image/4_4_2_Red_3.PNG}
	\caption{APIOA2\_Application\_Record}
	\label{figura:apioa2_application_record}
\end{figure}
...
\subsubsection{APIOA2\_AUTHORIZATION\_BASE}
\label{subsubsec:apioa2_authorization_base}

La figura \ref{figura:apioa2_authorization_base} es un ejemplo de la forma que va a tener la \gls{bbdd}.

\begin{figure}[H]
	\includegraphics[scale=1, center]{image/4_4_2_Red_4.PNG}
	\caption{APIOA2\_Authorization\_Base}
	\label{figura:apioa2_authorization_base}
\end{figure}

Tabla que almacena el paso intermedio en el que se recibe un código de autorización correcto.
\begin{itemize}
	\item \textbf{client\_id}: El CLIENT\_ID lo emite la API y actúa como usuario a la hora de solicitar recursos.
	\item \textbf{authorization\_code}: Clave intermedia en el proceso de autorización.
\end{itemize}
...
Abajo se pueden observar iconos de las diferentes Redes Sociales (\gls{rrss}). Aquellos que estén en azul redireccionarán a la página personal del creador de este proyecto, en la \gls{rrss} específica.  Estos iconos, junto con el 'Copyright' se ha establecido de esta manera para dar un aspecto atractivo y de empaque a la página principal.\\
...
La página principal se encuentra en el archivo \textit{index.html} \href{}{\texttt{}}
(\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/webapp/index.html}{\texttt{https://github.com/\linebreak Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/\linebreak webapp/index.html}}). Al hacer click en el botón 'Login/Register' se redirecciona al JSP \textit{login.jsp} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/webapp/login.jsp}{\texttt{https://github.com/Xevit/eclipse-workspace/blob/master/\linebreak ApplicationOAuth2/src/main/webapp/login.jsp}}). Este JSP tiene cargado un script llamado \textit{ajax\_json\_log.js} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/webapp/js/ajax_json_log.js}{\texttt{https://github.com/Xevit/eclipse-workspace/blob/\linebreak master/ApplicationOAuth2/src/main/webapp/js/ajax\_json\_log.js}}) con dos funciones Ajax. La primera de ellas se activará al realizar el login y la segunda al realizar el registro.

\begin{enumerate}
	\item La primera redireccionará a la URL \textbf{/login\_usuario} que estará gestionada por la clase \textit{LoginServlet.java} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/application/LoginServlet.java}{\texttt{https://github.com/Xevit/eclipse-workspace/blob/\linebreak master/ApplicationOAuth2/src/main/java/com/ptellos/\linebreak application/LoginServlet.java}}). En esta clase se comprobará que exista el usuario y contraseña que nos envían. Para comprobar la existencia del usuario se llamará al método de la clase \textit{DAOLogin.existUser} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/dao/DAOLogin.java}{\texttt{https://github.com/Xevit/eclipse-\linebreak workspace/blob/master/ApplicationOAuth2/src/main/java/com/\linebreak ptellos/dao/DAOLogin.java}}):
	\begin{enumerate}
		\item En caso de existir se creará una cookie (básica) para la persistencia de la sesión y se devolverá la resolución de la clase al archivo \textit{ajax\_json\_log.js} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/webapp/js/ajax_json_log.js}{\texttt{https://github\linebreak .com/Xevit/eclipse-workspace/blob/master/\linebreak ApplicationOAuth2/src/main/webapp/js/ajax\_json\_log.js}}). Este redireccionará a la página principal. En la clase principal se podrá observar como se dispone de una sesión abierta con el usuario logueado además de tener disponible los botones de interacción con la aplicación. En caso de haberse realizado el login con el usuario 'admin' se dispondrá de los botones de Alta y Baja de Aplicaciones en la API.
		\item En caso de no existir se devolverá la resolución de la clase al archivo \textit{ajax\_json\_log.js} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/webapp/js/ajax_json_log.js}{\texttt{https://github.com/Xevit/eclipse-workspace/blob/master/\linebreak ApplicationOAuth2/src/main/webapp/js/ajax\_json\_log.js}}). Este mostrará un mensaje indicando que el usuario y/o contraseña no son válidos.
	\end{enumerate}
	\item La segunda redireccionará a la URL \textbf{/registra\_usuario} que estará gestionada por la clase \textit{RegisterServlet.java} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/application/RegisterServlet.java}{\texttt{https://github.com/Xevit/eclipse-workspace/\linebreak blob/master/ApplicationOAuth2/src/main/java/com/ptellos/\linebreak application/RegisterServlet.java}}). En esta clase se comprobará que el usuario no existe a través del método de la clase \textit{DAORegistro.existUser} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/application/RegisterServlet.java}{\texttt{https://github\linebreak .com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/\linebreak src/main/java/com/ptellos/application/RegisterServlet.java}}):
	\begin{enumerate}
		\item En caso de que el usuario ya exista se finalizará el hilo de la clase y se devolverá la resolución al archivo \textit{ajax\_json\_log.js} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/webapp/js/ajax_json_log.js}{\texttt{https://github.com/Xevit/\linebreak eclipse-workspace/blob/master/ApplicationOAuth2/src/\linebreak main/webapp/js/ajax\_json\_log.js}}). Este mostrará un mensaje indicando que el usuario que se está intentando dar de alta ya existe en la Aplicación.
		\item En caso de que el usuario no exista se realizará una llamada al método de la clase \textit{DAORegistro.setUser} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/dao/DAORegistro.java}{\texttt{https://github.com/Xevit/eclipse-\linebreak workspace/blob/master/ApplicationOAuth2/src/main/java/\linebreak com/ptellos/dao/DAORegistro.java}}) para que realice una inserción en \gls{bbdd} de todos los campos que ha facilitado el usuario. Posteriormente finalizará el hilo de la clase y se devolverá la resolución al archivo \textit{ajax\_json\_log.js} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/webapp/js/ajax_json_log.js}{\texttt{https://github.com/Xevit/eclipse-workspace/blob/master/\linebreak ApplicationOAuth2/src/main/webapp/js/ajax\_json\_log.js}}). Este mostrará un meensaje indicando que el usuario ha sido creado satisfactoriamente.
	\end{enumerate}
\end{enumerate}
...
\begin{enumerate}
	...
	\item Se manda la primera solicitud directa entre aplicación y API. Dicha solicitud se realizará mediante \gls{rest} y llevará en la URL la aplicación que la realiza, así como el código en Base64.
	\item La API lee la URL y va a buscar en su \gls{bbdd} si existe alguna aplicación dada de alta que corresponda con los datos que se le mandan en la petición.
		\begin{enumerate}
			...
			\item En caso de que la aplicación no estuviera dada de alta en la API se generará un código \textbf{CLIENT\_ID} y un código \textbf{CLIENT\_SECRET}. Se almacenará en la \gls{bbdd} de la API toda la información correspondiente a la misma y se responderá indicando que dicha aplicación no estaba dada de alta en la API y facilitándole los dos códigos generados.
		\end{enumerate}
	...
	\begin{enumerate}
		...
		\end{figure}
	\end{enumerate}
	\item Para finalizar, en caso de que la aplicación no estuviera dada de alta se almacenarán los datos facilitados por la API en la \gls{bbdd} de la aplicación.
\end{enumerate}
...
\begin{enumerate}
	\item Se manda una solicitud directa desde la aplicación a la API. Dicha solicitud se realizará mediante \gls{rest} y llevará en la URL la aplicación que la realiza, así como el código inicial en Base64.
	\item La API lee la URL y va a buscar en su \gls{bbdd} si existe alguna aplicación dada de alta que corresponda con los datos que se le mandan en la petición.
		\begin{enumerate}
			\item En caso de que la aplicación estuviera dada de alta se borrará de la \gls{bbdd} de la API toda la información correspondiente a la misma y se responderá indicando que dicha aplicación estaba dada de alta en la API.
	...
	\item En la parte de la aplicación se nos indicará el resultado del proceso de baja:
		\begin{enumerate}
			...
			\end{figure}
		\end{enumerate}
	\item Para finalizar, en caso de que la aplicación estuviera dada de alta se borrarán los datos relativos a este registro en la \gls{bbdd} de la aplicación.
\end{enumerate}
...
\begin{enumerate}
	\item Se llama al archivo \textit{apiModal.js} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/webapp/js/apiModal.js}{\texttt{https://github.com/Xevit/eclipse\linebreak -workspace/blob/master/ApplicationOAuth2/src/main/webapp/js/\linebreak apiModal.js}}) que tendrá una función Ajax mediante la cuál mandaremos un mensaje POST al recurso \textbf{ApplicationOAuth2/AltaEnAPI} que se encuentra en la clase \textit{ApplicationRegistering.java} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/application/ApplicationRegistering.java}{\texttt{https://github.com/Xevit/eclipse-workspace/\linebreak blob/master/ApplicationOAuth2/src/main/java/com/ptellos/\linebreak application/ApplicationRegistering.java}}) de la Aplicación.
	\item Desde la clase \textit{ApplicationRegistering.java} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/application/ApplicationRegistering.java}{\texttt{https://github.com/Xevit/eclipse\linebreak -workspace/blob/master/ApplicationOAuth2/src/main/java/com/\linebreak ptellos/application/ApplicationRegistering.java}}) se llamará a la siguiente URL mediante mensaje GET: \\
	\href{}{\texttt{}}
	...
	\item La anterior URL será manejada por la clase \textit{ListenerRegistrationRequest.java} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/api/ListenerRegistrationRequest.java}{\texttt{https://\linebreak github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/\linebreak main/java/com/ptellos/api/ListenerRegistrationRequest.java}}) de la API que llamará al método \textit{DAORegisterApplication.existApp} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/dao/DAORegisterApplication.java}{\texttt{https://github.com\linebreak /Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/\linebreak com/ptellos/dao/DAORegisterApplication.java}}) para comprobar si existe o no la aplicación.
		\begin{enumerate}
			\item Si la aplicación ya estaba dada de alta, se enviará el siguiente mensaje GET:\\
			\textbf{\href{http://localhost:8080/ApplicationOAuth2/AltaEnAPI/Confirmation?exist=true}{\texttt{http://localhost:8080/ApplicationOAuth2/AltaEnAPI/\linebreak Confirmation?exist=true}}}
			\item Si la aplicación no estaba dada de alta:
				\begin{enumerate}
					\item Se llama a la clase \textit{RandomStringGenerator.java} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/api/RandomStringGenerator.java}{\texttt{https://github.com/\linebreak Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/\linebreak java/com/ptellos/api/RandomStringGenerator.java}}) para generar el CLIENT\_ID y el CLIENT\_SECRET.
					\item Se llama al método \textit{DAORegisterApplication.registerApplication} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/dao/DAORegisterApplication.java}{\texttt{https://\linebreak github.com/Xevit/eclipse-workspace/blob/master/\linebreak APIOAuth2/src/main/java/com/ptellos/dao/\linebreak DAORegisterApplication.java}}) para almacenar todos los valores en la \gls{bbdd} de la API.
					\item Se envia el siguiente mensaje GET:\\
					\textbf{\href{http://localhost:8080/ApplicationOAuth2/AltaEnAPI/
				...
				\end{enumerate}	
		\end{enumerate}
	\item Tanto la respuesta de que la clase ya estaba creada como la de que no estaba creada se manejarán desde el recurso \textbf{ApplicationOAuth2/AltaEnAPI} en el método que maneja las peticiones GET de la clase \textit{ApplicationRegistering.java} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/application/ApplicationRegistering.java}{\texttt{https://github.com/\linebreak Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/\linebreak main/java/com/ptellos/application/\linebreak ApplicationRegistering.java}}). 
		\begin{enumerate}
			...
			\item En caso de que la aplicación no estuviera dada de alta:
				\begin{enumerate}
					\item Se llamará al método \textit{DAORegisterApplication.registerApplication} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/dao/DAORegisterApplication.java}{\texttt{https://\linebreak github.com/Xevit/eclipse-workspace/blob/master/\linebreak ApplicationOAuth2/src/main/java/com/ptellos/dao/\linebreak DAORegisterApplication.java}}) para almacenar todos los valores en la \gls{bbdd} de la aplicación.
...
\begin{enumerate}
	\item Se llama al archivo \textit{apiModal.js} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/webapp/js/apiModal.js}{\texttt{https://github.com/Xevit/eclipse\linebreak -workspace/blob/master/ApplicationOAuth2/src/main/webapp/js/\linebreak apiModal.js}}) que tendrá una función Ajax mediante la cuál mandaremos un mensaje POST al recurso \textbf{ApplicationOAuth2/BajaEnAPI} que se encuentra en la clase \textit{ApplicationUnsubscribe.java} de la Aplicación.
	\item Desde la clase \textit{ApplicationUnsubscribe.java} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/application/ApplicationUnsubscribe.java}{\texttt{https://github.com/Xevit/\linebreak eclipse-workspace/blob/master/ApplicationOAuth2/src/main/\linebreak java/com/ptellos/application/ApplicationUnsubscribe.java}}) se llamará a la siguiente URL mediante mensaje GET: \\
	\href{}{\texttt{}}
	...
	\item La anterior URL será manejada por la clase \textit{ListenerUnsubscribeRequest.java} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/api/ListenerUnsubscribeRequest.java}{\texttt{https://\linebreak github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/\linebreak src/main/java/com/ptellos/api/ListenerUnsubscribeRequest.java}}) de la API que llamará al método \textit{DAOUnsubscribeApplication.existApp} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/dao/DAOUnsubscribeApplication.java}{\texttt{https://\linebreak github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/\linebreak main/java/com/ptellos/dao/DAOUnsubscribeApplication.java}}) para comprobar si existe o no la aplicación.
		\begin{enumerate}
			\item Si la aplicación estaba dada de alta:
				\begin{enumerate}
					\item  Se llama al método \textit{DAOUnsubscribeApplication.unsubscribeApplication} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/dao/DAOUnsubscribeApplication.java}{\texttt{https:\linebreak //github.com/Xevit/eclipse-workspace/blob/master/\linebreak APIOAuth2/src/main/java/com/ptellos/dao/\linebreak DAOUnsubscribeApplication.java}}) para eliminar todos los valores en la \gls{bbdd} de la API.
					\item Se envía el siguiente mensaje GET:\\
					\textbf{\href{http://localhost:8080/ApplicationOAuth2/BajaEnAPI/Confirmation?exist=true&url_redirect=http://localhost:8080/APIOAuth2}{\texttt{http://localhost:8080/ApplicationOAuth2/BajaEnAPI/\linebreak Confirmation?exist=true\&url\_redirect=http://localhost\linebreak :8080/APIOAuth2}}}
				\end{enumerate} 
			\item Si la aplicación no estaba dada de alta, se enviará el siguiente mensaje GET: \\
			\href{}{\texttt{}}
			\textbf{\href{http://localhost:8080/ApplicationOAuth2/BajaEnAPI/Confirmation?exist=false}{\texttt{http://localhost:8080/ApplicationOAuth2/BajaEnAPI/\linebreak Confirmation?exist=false}}}
		\end{enumerate}
	\item Tanto la respuesta de que la clase ya estaba creada como la de que no estaba creada se manejarán desde el recurso \textbf{ApplicationOAuth2/BajaEnAPI} en el método que maneja las peticiones GET de la clase \textit{ApplicationUnsubscribe.java} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/application/ApplicationUnsubscribe.java}{\texttt{https://github.com/\linebreak Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/\linebreak main/java/com/ptellos/application/\linebreak ApplicationUnsubscribe.java}}).
		\begin{enumerate}
			\item En caso de que la aplicación estuviera dada de alta:
				\begin{enumerate}
					\item Se llamará al método \textit{DAOUnsubscribeApplication.java} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/dao/DAOUnsubscribeApplication.java}{\texttt{https://\linebreak github.com/Xevit/eclipse-workspace/blob/master/\linebreak ApplicationOAuth2/src/main/java/com/ptellos/dao/\linebreak DAOUnsubscribeApplication.java}}) para eliminar todos los valores de dicho registro en la \gls{bbdd} de la aplicación. 
...
\begin{enumerate}
	...
	\end{figure}
	\item La API gestionará dicha solicitud y buscará en su \gls{bbdd} si la aplicación que está generando dicha solicitud está dada de alta en la API. 
		\begin{enumerate}
	...
	\item La API comprobará buscando en su \gls{bbdd} si el usuario que está intentando hacer login, existe. En caso de que exista se generará el \textbf{AUTHORIZATION\_CODE} y se almacenará en la \gls{bbdd} junto con el \textbf{CLIENT\_ID}.
	...
	\item En caso de que la aplicación identifique, en la URL de respuesta, el \textbf{CLIENT\_ID} se gestionará dicha petición y almacenará en su \gls{bbdd} los datos recibidos. 
	...
	\item La API gestionará la petición entrante y en caso de que la petición obtenida sea correcta, generará un \textbf{ACCESS\_TOKEN} y almacenará todos los datos correpondientes a dicho proceso en su \gls{bbdd}.
	\item Se realizará un último envío desde la API que contendrá un \gls{json} con los datos correspondientes al proceso de concesión de autorización correcta entre los que se encontrarán:
		\begin{enumerate}
			\item \textbf{ACCESS\_TOKEN}: Token de acceso proporcionado por la API. 
		...
		\end{enumerate}
	\item Finalmente, para concluir el proceso de concesión de autorización, la aplicación almacenará en su \gls{bbdd} todos los datos enviados por la API.
\end{enumerate}
...
\begin{enumerate}
	\item Se pulsa el botón 'Grant' dentro de la página \textit{index.html} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/webapp/index.html}{\texttt{https://github.com/\linebreak Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/\linebreak main/webapp/index.html}}) que se gestionará en \textbf{ApplicationOAuth2/GrantAuthorization} dentro del método \textit{ApplicationGrant.doGet()} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/application/ApplicationGrant.java}{\texttt{https://github.com/Xevit/\linebreak eclipse-workspace/blob/master/ApplicationOAuth2/src/main/\linebreak java/com/ptellos/application/ApplicationGrant.java}}). Se preparará la redirección con la siguiente URL:\\
	\textbf{\url{http://localhost:8080/APIOAuth2/GrantAuthorization/FirstStep?response_type=code&client_id=[CLIENT_ID]&redirect_uri=http://localhost:8080/ApplicationOAuth2/ResponseAuthorization&scope=read}}
	\item El método \textit{ListenerGrant.doGet()} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/api/ListenerGrant.java}{\texttt{https://github.com/Xevit/\linebreak eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/\linebreak ptellos/api/ListenerGrant.java}}) será el encargada de analizar la respuesta y solo se analizará si el \textbf{response\_type} es del tipo \textbf{code}. Se comprobará que la aplicación está dada de alta en la API a través del método \textit{DAOSearchApplication.existApp} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/dao/DAOSearchApplication.java}{\texttt{https://github.com/Xevit/eclipse-workspace/blob/master/\linebreak APIOAuth2/src/main/java/com/ptellos/dao/\linebreak DAOSearchApplication.java}}).
		\begin{enumerate}
			\item En caso de que sí esté dada de alta, se redireccionará a la página:\\
			\textbf{\href{http://localhost:8080/APIOAuth2/login.jsp?client_id=[CLIENT_ID]}{\texttt{http://localhost:8080/APIOAuth2/login.jsp?\linebreak client\_id=[CLIENT\_ID]}}}
			...
			\textbf{\url{http://localhost:8080/APIOAuth2/loginOff.html}}
		\end{enumerate}
	\item La página \textit{login.jsp} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/webapp/login.jsp}{\texttt{https://github.com/Xevit/eclipse-workspace/blob/\linebreak master/APIOAuth2/src/main/webapp/login.jsp}}) recogerá el nombre de usuario y contraseña y mediante una función Ajax en el archivo \textit{login.js} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/webapp/js/login.js}{\texttt{https://github.\linebreak com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/\linebreak webapp/js/login.js}}) enviará los datos a través de un mensaje [POST] a la URL: \\
	\textbf{\url{http://localhost:8080/APIOAuth2/ProcessGrant}}
	\begin{enumerate}
		\item En el método \textit{ProcessGrant.doPost()} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/api/ProcessGrant.java}{\texttt{https://github.com/Xevit/\linebreak eclipse-workspace/blob/master/APIOAuth2/src/main/java/\linebreak com/ptellos/api/ProcessGrant.java}}) se comprobará si el usuario existe en la API a través del método \textit{DAOLogin.existUser} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/dao/DAOLogin.java}{\texttt{https://github.com/\linebreak Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/\linebreak java/com/ptellos/dao/DAOLogin.java}}) .
		\begin{enumerate}
			\item En caso de que exista se volverá a la función Ajax del archivo \textit{login.js} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/webapp/js/login.js}{\texttt{https:\linebreak //github.com/Xevit/eclipse-workspace/blob/master/\linebreak APIOAuth2/src/main/webapp/js/login.js}}) y se llamará a la función \textit{callProcess} que redireccionará mediante el método [GET] a:\\
			\textbf{\url{http://localhost:8080/APIOAuth2/ProcessGrant}}
		\end{enumerate}
		\item En el método \textit{ProcessGrant.doGet()} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/api/ProcessGrant.java}{\texttt{https://github.com/Xevit/eclipse\linebreak -workspace/blob/master/APIOAuth2/src/main/java/com/\linebreak ptellos/api/ProcessGrant.java}}):
			\begin{enumerate}
				\item Se generará el \textbf{AUTHORIZATION\_CODE} a través de la clase \textit{RandomStringGenerator()} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/api/RandomStringGenerator.java}{\texttt{https://github.com/Xevit/eclipse-workspace/\linebreak blob/master/APIOAuth2/src/main/java/com/ptellos/api/\linebreak RandomStringGenerator.java}}).  
				\item Se comprueba que existe el \textbf{CLIENT\_ID} a través del método \textit{DAOSetAuthorizationCode.existApp} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/dao/DAOSetAuthorizationCode.java}{\texttt{https://github.com/Xevit/eclipse\linebreak -workspace/blob/master/APIOAuth2/src/main/java/com/\linebreak ptellos/dao/DAOSetAuthorizationCode.java}}) y si existe se añadirá la dupla \textbf{CLIENT\_ID} / \textbf{AUTHORIZATION\_CODE} a la \gls{bbdd} de la API a través del método \textit{DAOSetAuthorizationCode.setApplication} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/dao/DAOSetAuthorizationCode.java}{\texttt{https://github\linebreak .com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/\linebreak main/java/com/ptellos/dao/DAOSetAuthorizationCode.java}}).
				...
				\textbf{\href{http://localhost:8080/ApplicationOAuth2/ResponseAuthorization?code=[AUTHORIZATION_CODE]&client_id=[CLIENT_ID]}{\texttt{http://localhost:8080/ApplicationOAuth2/\linebreak ResponseAuthorization?code=[AUTHORIZATION\_CODE]\&\linebreak client\_id=[CLIENT\_ID]}}}
			\end{enumerate}
	\end{enumerate} 
	\item El método \textit{ResponseAuthorization.doGet()} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/application/ResponseAuthorization.java}{\texttt{https://github.com/Xevit/\linebreak eclipse-workspace/blob/master/ApplicationOAuth2/src/main/\linebreak java/com/ptellos/application/ResponseAuthorization.java}}) en el lado de la aplicación, procesará la respuesta:
		\begin{enumerate}
			\item Se comprobará que el \textbf{CLIENT\_ID} existe en la \gls{bbdd} de la aplicación mediante el método \textit{DAOSearch.existClient} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/dao/DAOSearch.java}{\texttt{https://github.com/Xevit/\linebreak eclipse-workspace/blob/master/ApplicationOAuth2/src/\linebreak main/java/com/ptellos/dao/DAOSearch.java}}).
			\item Se solicitará el \textbf{CLIENT\_SECRET} mediante el método \textit{DAOSearch.returnClientSecret} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/dao/DAOSearch.java}{\texttt{https://github.com/Xevit/eclipse-workspace/blob/master/\linebreak ApplicationOAuth2/src/main/java/com/ptellos/dao/\linebreak DAOSearch.java}}).
		...
		\end{enumerate}	
	\item El método \textit{ListenerGrant.doGet()} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/api/ListenerGrant.java}{\texttt{https://github.com/Xevit/eclipse\linebreak -workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/\linebreak api/ListenerGrant.java}}) en el lado de la API, procesará la respuesta:
	\begin{enumerate}
		\item Se comprueba a través del método \textit{DAOSearch.existApp} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/dao/DAOSearch.java}{\texttt{https://github.com\linebreak /Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main\linebreak /java/com/ptellos/dao/DAOSearch.java}}) en una primera instancia si se tiene almacenado alguna aplicación que disponga de un \textbf{CLIENT\_ID} asociado al \textbf{AUTHORIZATION\_CODE} que nos están remitiendo y si eso se cumple, en una segunda instancia se comprobará si existe un \textbf{CLIEN\_ID} asociado al \textbf{CLIENT\_SECRET} que nos están facilitando.
		\begin{enumerate}
			\item En caso de que se cumpla a través del método \textit{DAOInsert.setCorrectGrant} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/dao/DAOInsert.java}{\texttt{https://github.com/Xevit/eclipse-workspace/blob/master\linebreak /APIOAuth2/src/main/java/com/ptellos/dao/DAOInsert.java}}) se almacenará toda la información sobre dicha concesión de autorización.
		\end{enumerate} 
		\item Mediante el método \textit{ListenerGrant.responsePost()} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/java/com/ptellos/api/ListenerGrant.java}{\texttt{https://github.com/\linebreak Xevit/eclipse-workspace/blob/master/APIOAuth2/src/main/\linebreak java/com/ptellos/api/ListenerGrant.java}}) se creará un \gls{json} que albergue toda la información para la finalización del proceso de concesión de autorización. Dicho \gls{json} contendrá los siguientes datos:\\
		\textbf{\href{request: {'access_token': '[ACCESS_TOKEN]','token_type': 'bearer','expires_in': '[EXPIRES_IN]','refresh_token': '[REFRESH_TOKEN]','scope': 'read'}}{\texttt{request:\linebreak \{'access\_token': '[ACCESS\_TOKEN]',\linebreak 'token\_type': 'bearer',\linebreak 'expires\_in': '[EXPIRES\_IN]',\linebreak 'refresh\_token': '[REFRESH\_TOKEN]',\linebreak 'scope': 'read'\}}}}
		\item Se realizará mediante un método [POST] la siguiente llamada:\\
		\textbf{\url{http://localhost:8080/ApplicationOAuth/GrantAuthorization}}
	\end{enumerate}
	\item El método \textit{ApplicationGrant.doPost()} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/application/ApplicationGrant.java}{\texttt{https://github.com/Xevit/eclipse\linebreak -workspace/blob/master/ApplicationOAuth2/src/main/java/com/\linebreak ptellos/application/ApplicationGrant.java}}) en el lado de la aplicación, procesará la respuesta:
	\begin{enumerate}
		\item Se extraerá la información contendia en el \gls{json} y mediante el método \textit{DAOValueApplicationRequest.setApplicationCredentials} (\href{https://github.com/Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/main/java/com/ptellos/dao/DAOValueApplicationRequest.java}{\texttt{https://github.com/\linebreak Xevit/eclipse-workspace/blob/master/ApplicationOAuth2/src/\linebreak main/java/com/ptellos/dao/DAOValueApplicationRequest.java}}) se inicializarán dichos parámetros en la \gls{bbdd}.
	\end{enumerate}
\end{enumerate}
...
\begin{enumerate}
	\item Realización de un proceso de autenticación de usuarios en la aplicación principal. Para esto se requería que los datos del usuario estuvieran almacenados en una \gls{bbdd} a la cuál se tenía que acceder y comprobar si el nombre de usuario y contraseña estaban dados de alta como dupla. Se realiza la gestión tanto de los accesos correctos como de aquellos que resultan erróneos.
...
En los capítulos anteriores se ha presentado el estudio de la documentación y la motivación de realizar este proyecto. También se ha desarrollado una implementación de dicho protocolo (\gls{rfc} 6749)~\cite{hardt2012rfc} y analizado el comportamiento bajo diversas circunstancias.
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item La realización de un mecanismo de autenticación de usuarios. Dicho mecanismo podría basarse en Infraestructura de Clave Pública y combinarlo con otro mecanismo como puede ser el mecanismo \gls{otp} (One Time Password) que añade un nivel de seguridad adicional al solicitar una clave que se envía al teléfono o correo electrónico que tenga asociado el usuario.
