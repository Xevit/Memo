...
La seguridad de la información en internet consta de dos aspectos bien diferenciados, por una parte podemos encontrar la autenticación y por otra la autorización. Mientras que la autenticación trata de demostrar que una persona es quién dice ser, la autorización trata de discernir el grado de confianza que se concede para la obtención de un recursos o conjunto de recursos específico. El objetivo principal de este proyecto es la realización de un mecanismo de autorización basado en el protocolo OAuth 2.0 (RFC 6749).
...
The security of information on the Internet consists of two distinct aspects, on the one hand we can find authentication and on the other authorization. While authentication tries to prove that a person is who they say they are, the authorization tries to discern the degree of trust that is granted to obtain a specific resource or set of resources. The main objective of this project is the realization of an authorization mechanism based on the protocol OAuth 2.0 (RFC 6749).

This implementation has been done using Java, JavaScript, HTML and CSS. The application server WildFly (version 13) was used to deploy the application. As a database, PostgreSQL has been used.

This project has been carried out in the field of information security and cybersecurity and tries to demonstrate the positive aspects of good management in the authorization of resources, such as the one carried out by the protocol orAuth 2.0.
...
Este Trabajo Fin de Grado se desarrolla en el ámbito de la seguridad y la preservación de la autenticación y de la autorización en Internet. En este capítulo introduciremos los conceptos y tecnologías utilizadas a la vez que se presentará el esquema general de la custodia de la información.
%\\[1\baselineskip]
\section{Historia}
\label{sec:historia}
...
El estándar OAuth 2.0 (RFC 6749) es un \emph{framework} de autorización que permite controlar el acceso por parte de las aplicaciones a los datos de los usuarios sin tener que proporcionar las credenciales. Según el estándar, el marco de autorización de OAuth 2.0 permite que una aplicación de terceros obtenga acceso limitado a un servicio HTTP, ya sea en nombre de un propietario de recursos mediante la organización de una interacción de aprobación entre el propietario del recurso y el servicio HTTP, o permitiendo a la aplicación de terceros obtener acceso en su propio nombre. 
...
\subsection{Ajax}
\label{subsec:ajax}
Ajax\footnote{\url{https://web.archive.org/web/20080413132458/http://www.librosweb.es/ajax/index.html}} acrónimo de Asynchronous JavaScript And XML (JavaScript asíncrono y XML), es una técnica de desarrollo web para crear aplicaciones interactivas o RIA (Rich Internet Applications). Estas aplicaciones se ejecutan en el cliente, es decir, en el navegador de los usuarios mientras se mantiene la comunicación asíncrona con el servidor en segundo plano. De esta forma es posible realizar cambios sobre las páginas sin necesidad de recargarlas, mejorando la interactividad, velocidad y usabilidad en las aplicaciones.
...
Este esquema muestra el alta de la Aplicación en la API. Los diferentes pasos que se dan hasta llegar a dicho propósito serían los siguientes:
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item El siguiente mensaje es el que se obtendrá en caso de que el alta se haya realizado correctamente.
		\begin{figure}[H]
		...
		\end{figure}
		\item El siguiente mensaje se obtendrá en caso de que la aplicación ya estuviera dada de alta en la API.
		\begin{figure}[H]
...
Este esquema muestra la baja de la aplicación en la API. Los diferentes pasos que se dan hasta llegar a dicho propósito serían los siguientes:
...
\begin{enumerate}
	...
	\item En la parte de la aplicación se nos indicará el resultado del proceso de baja:
		\begin{enumerate}
			\item El siguiente mensaje es el que se obtendrá en caso de que la baja se haya realizado correctamente.
			\begin{figure}[H]
			...
			\end{figure}
			\item El siguiente mensaje se obtendrá en caso de que la baja no se haya podido realizar porque la aplicación no estuviera dada de alta en la API.
			\begin{figure}[H]
...
\begin{enumerate}
	...
	\textbf{\url{http://localhost:8080/APIOAuth2/Register?url\_redirect=http://localhost:8080/ApplicationOAuth2/AltaEnAPI/Confiramation?code\_secret=[CODE\_SECRET]}}
	\item La anterior URL será manejada por la clase \textit{ListenerRegistrationRequest.java} de la API que llamará al método \textit{DAORegisterApplication.existApp} para comprobar si existe o no la aplicación.
		\begin{enumerate}
			...
			\textbf{\url{http://localhost:8080/ApplicationOAuth2/AltaEnAPI/Confirmation?exist=true}}
			\item Si la aplicación no estaba dada de alta:
				\begin{enumerate}
					...
					\textbf{\url{http://localhost:8080/ApplicationOAuth2/AltaEnAPI/Confirmation?exist=false&clientId=[CLIENT\_ID]&clientSecret=[CLIENT\_SECRET]&url_redirect=http://localhost:8080/APIOAuth2}}
				\end{enumerate}	
		\end{enumerate}
...
\begin{enumerate}
	...
	\textbf{\url{http://localhost:8080/APIOAuth2/Unsubscribe?url_redirect=http://localhost:8080/ApplicationOAuth2/BajaEnAPI/Confirmation?code\_secret=[CODE\_SECRET]}}
		...
		\begin{enumerate}
			\item Si la aplicación estaba dada de alta:
				\begin{enumerate}
					...
					\textbf{\url{http://localhost:8080/ApplicationOAuth2/BajaEnAPI/Confirmation?exist=true&url_redirect=http://localhost:8080/APIOAuth2}}
				\end{enumerate} 
			\item Si la aplicación no estaba dada de alta, se enviará el siguiente mensaje GET: \\
			\textbf{\url{http://localhost:8080/ApplicationOAuth2/BajaEnAPI/Confirmation?exist=false}}
		\end{enumerate}
...
Este esquema va a ayudar a entender todo el proceso de la concesión de autorización. Desde que se realiza la petición en la aplicación hasta que terminamos obteniendo las credenciales necesarias.
...
\begin{enumerate}
	...
	\begin{figure}[H]
		\includegraphics[scale=0.40, center]{image/5_1_1_Login_3_OK.PNG}
	...
	\item La API gestionará dicha solicitud y buscará en su BBDD si la aplicación que está generando dicha solicitud está dada de alta en la API. 
		\begin{enumerate}
			...
			\item En caso contrario se lanzará la siguiente ventana, indicando que la aplicación no dispone de permisos para la petición que está lanzando, tal y como se muestra en la siguiente imagen:
			\begin{figure}[H]
				\includegraphics[scale=0.50, center]{image/5_3_Web_API_KO.PNG}
				\caption{La aplicación no dispone de permisos}
				\label{figura:oAuth2_2}
			\end{figure}
		\end{enumerate}
	\clearpage
	\item En caso de estar dada de alta en la API se lanzará una web de login mediante la cuál el usuario que está intentando dar autorización a la aplicación se autenticará. 
	\begin{figure}[H]
...
\begin{enumerate}
	...
	\item El método \textit{ListenerGrant.doGet()} será el encargada de analizar la respuesta y solo se analizará si el \textbf{response\_type} es del tipo \textbf{code}. Se comprobará que la aplicación está dada de alta en la API a través del método \textit{DAOSearchApplication.existApp}.
		\begin{enumerate}
			...
			\textbf{\url{http://localhost:8080/APIOAuth2/login.jsp?client_id=[CLIENT_ID]}}
	...
	\begin{enumerate}
		...
		\item En el método \textit{ProcessGrant.doGet()}:
			\begin{enumerate}
				...
				\textbf{\url{http://localhost:8080/ApplicationOAuth2/ResponseAuthorization?code=[AUTHORIZATION_CODE]&client_id=[CLIENT_ID]}}
			\end{enumerate}
	\end{enumerate} 
	\item El método \textit{ResponseAuthorization.doGet()} en el lado de la aplicación, procesará la respuesta:
		\begin{enumerate}
			...
			\textbf{\url{http://localhost:8080/APIOAuth2/GrantAuthorization/SecondStep?client_id=[CLIENT_ID]&client_secret=[CLIENT_SECRET]&code=[AUTHORIZATION_CODE]&redirect_uri=http://localhost:8080/ApplicationOAuth/GrantAuthorization}}
		\end{enumerate}	
	...
	\begin{enumerate}
		...
		\textbf{\url{request: {'access_token': '[ACCESS_TOKEN]','token_type': 'bearer','expires_in': '[EXPIRES_IN]','refresh_token': '[REFRESH_TOKEN]','scope': 'read'}}}
...
\begin{enumerate}
	\item Realización de un proceso de autenticación de usuarios en la aplicación principal.
	\item Realización de un proceso de alta y baja de aplicaciones en la API.
...
En los capítulos anteriores se ha presentado el estudio de la documentación y la motivación de realizar este proyecto. También se ha desarrollado una implementación de dicho protocolo (RFC 6749) y analizado el comportamiento bajo diversas circunstancias.
